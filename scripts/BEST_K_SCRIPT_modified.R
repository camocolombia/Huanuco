...
#bestk(data=x,ini=2,iter=100,kmax=20,type="hierarchical",dist.met="gower")
...
bestk=function(data,ini,iter,kmax,type,dist.met){
  ...
  for(j in ini:kmax){
    indexs=as.data.frame(matrix(0,iter,3))
    for(i in 1:iter){
      ...
      if(type=="hierarchical"){
        ...
        if (dist.met=="gower"){
          ...
          cat("hierarchical cluster approach"," Gower distance used","\n")
          
          X<-hclust(daisy(data,metric = "gower"),method = "ward.D")#NEW
          X2<-cutree(X,k=j)#h=10#NEW
          X$cluster<-as.numeric(X2)#NEW
          X$size<-as.numeric(tapply(X$cluster,X$cluster,length))
          #Y <-cclust(data,j,100,verbose=FALSE,method="kmeans")
          
          Y<-hclust(daisy(data,metric = "gower"),method = "ward.D")#NEW
          Y2<-cutree(Y,k=j)#h=10#NEW
          Y$cluster<-as.numeric(Y2)#NEW
          Y$size<-as.numeric(tapply(Y$cluster,Y$cluster,length))#NEW      
      ...
      if(is.na(indexqual(data,X$cluster,j)) &
         is.na(elbow(data,X$cluster,j))){
        indexs[i,]=c(indexsim(X$cluster,Y$cluster,j),NA,NA)
        clustqual = NA
        clustelbow = NA
      } else {
