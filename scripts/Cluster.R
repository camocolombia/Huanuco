...
sc <- scale(tab3_alt2[,c(1:7,14:27)],scale = T,center=F)
...
plot(2:150, sapply(2:150, function(i) { 
  cat(i,"\n")
...
#ClusterEvents_sc      <- hclust(gow.mat_sc,method="ward.D")
clust_per <- cutree(ClusterEvents_sc,h=sort(ClusterEvents_sc$height,decreasing = T)[4])
...
factoextra::fviz_dend(ClusterEvents_sc,k=4, rect = TRUE, cex = 0.5,
